...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-brand-sand flex items-center justify-center p-3 sm:p-6 font-sans">
      
      {/* Smartphone Mockup Container */}
      <div className="relative bg-brand-beige w-full max-w-[420px] h-[780px] rounded-[42px] shadow-2xl border-[10px] border-brand-moss/10 flex flex-col justify-between overflow-hidden">
        ...
        <div className="flex-1 pt-6.5 px-4 pb-20 flex flex-col overflow-hidden relative">
          
          {/* Spacer below phone notch */}
          ...
          <div className="flex-1 flex flex-col overflow-hidden">
            ...
            {activeTab === 'firstaid' && <FirstAidView onGoToHome={() => setActiveTab('home')} />}
        ...
        <div className="absolute bottom-0 left-0 right-0 h-[88px] bg-white border-t border-brand-sand flex items-center justify-between px-3 pb-5 pt-2 z-40 shadow-[0_-4px_16px_rgba(0,0,0,0.03)]">
          {[
            { id: 'home', label: '首頁', icon: <Home className="w-6 h-6" /> },
            { id: 'journal', label: '日誌', icon: <BookOpen className="w-6 h-6" /> },
            { id: 'garden', label: '花園', icon: <Sprout className="w-6 h-6" /> },
            { id: 'gallery', label: '圖鑑', icon: <Library className="w-6 h-6" /> },
            { id: 'firstaid', label: '急救箱', icon: <ShieldAlert className="w-6 h-6" /> }
          ].map((tab) => {
            ...
            return (
              ...
              >
                ...
                <span className="text-[15px] tracking-tight font-black mt-1 font-sans whitespace-nowrap">
                  {tab.label}
                </span>
              </button>
            );
